...
const BG_FEAT = "#100d1f";
const TEXT_HI  = "#f1f0ff";
const TEXT_MID = "rgba(241,240,255,0.52)";
const TEXT_LOW = "rgba(241,240,255,0.27)";

// ── Número de WhatsApp para "¿Necesitas algo a medida?" ──────────────────────
...
export const PLANES: Plan[] = [
  ...
  {
    id: "catalogo-organizacion",
    nombre: "Catálogo y Organización",
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 5, marginBottom: 13 }}>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "clamp(22px,2.6vw,28px)", fontWeight: 700, color: TEXT_HI, letterSpacing: -1, lineHeight: 1 }}>
          {clp(plan.implementacion)}
        </span>
        <span style={{ fontSize: 11, color: TEXT_LOW, fontFamily: "'Outfit',sans-serif" }}>CLP</span>
      </div>

      <div style={{ height: 1, marginBottom: 13, background: `linear-gradient(90deg,${plan.color}22,rgba(255,255,255,.04),${plan.color}22)` }} />

      {/* Mensualidad */}
      <p style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, fontWeight: 700, letterSpacing: "0.13em", textTransform: "uppercase", color: TEXT_LOW, margin: "0 0 5px", display: "flex", alignItems: "center", gap: 5 }}>
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: "clamp(18px,2vw,22px)", fontWeight: 700, color: plan.esDestacado ? plan.color : "rgba(241,240,255,0.8)", letterSpacing: -0.5, lineHeight: 1 }}>
          {clp(plan.mensualidad)}
        </span>
        <span style={{ fontSize: 11, color: TEXT_LOW, fontFamily: "'Outfit',sans-serif" }}>/ mes</span>
      </div>
    </div>
  );
}
        ...
        <div style={{ padding: "26px 24px 28px", display: "flex", flexDirection: "column", flex: 1, position: "relative" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
            ...
            <h3 style={{
              ...
              fontSize: 16, fontWeight: 700, color: TEXT_HI,
          ...
          <p style={{
            fontFamily: "'Outfit',sans-serif",
            fontSize: 13, color: TEXT_MID, lineHeight: 1.65, margin: "0 0 18px",
          }}>
            {plan.descripcion}
          </p>
          ...
          <ul style={{
            ...
            {plan.incluye.map((item) => (
              <li key={item} style={{ display: "flex", alignItems: "flex-start", gap: 9 }}>
                ...
                <span style={{ fontFamily: "'Outfit',sans-serif", fontSize: 13, color: "rgba(241,240,255,0.65)", lineHeight: 1.5 }}>
                  {item}
                </span>
              </li>
            ))}
          ...
          {/* Botón → /contacto?plan=ID */}
          <button
            onClick={() => navigate(`/contacto?plan=${plan.id}`)}
            ...
            style={{
              ...
              ...(plan.esDestacado
                ? {
                    ...
                    boxShadow: hov ? `0 12px 32px ${plan.color}50` : `0 6px 20px ${plan.color}30`,
                  }
                : {
                    background: hov ? `${plan.color}12` : "transparent",
                    border: `1px solid ${hov ? plan.color + "55" : plan.color + "35"}`,
                    color: plan.color,
                  }),
            }}
          >
    ...
    <section id="planes" ref={ref} style={{ position: "relative", zIndex: 5, padding: "88px 24px 108px" }}>
      ...
      <div style={{ maxWidth: 1380, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <h2 style={{ fontFamily: "'Outfit',sans-serif", fontSize: "clamp(28px,5vw,48px)", fontWeight: 900, letterSpacing: -2, color: TEXT_HI, lineHeight: 1.05, margin: "0 0 18px" }}>
          ...
          <p style={{ fontFamily: "'Outfit',sans-serif", fontSize: 16, color: TEXT_MID, maxWidth: 480, margin: "0 auto", lineHeight: 1.7 }}>
        ...
        {/* Nota al pie con WhatsApp clickeable */}
        <p style={{
          fontFamily: "'Outfit',sans-serif",
          textAlign: "center", marginTop: 48,
          fontSize: 13.5, color: TEXT_LOW, lineHeight: 1.9,
          opacity: visible ? 1 : 0,
