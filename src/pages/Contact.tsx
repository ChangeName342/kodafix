    ...
    >
      ...
      >
        ...
        <div className="grid gap-12" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))" }}>
          ...
          {/* Formulario */}
          <div className="rounded-2xl p-8" style={{ background: "#0e0e1a", border: "1px solid rgba(255,255,255,0.06)" }}>
            {sent ? (
              ...
              <form onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <Field label="Motivo de contacto" error={errors.motivo}>
                  <select className={`contact-input${errors.motivo ? " err" : ""}`} value={form.motivo} onChange={(e) => handleChange("motivo", e.target.value)} style={{ appearance: "none", cursor: "pointer" }}>
                ...
                <Field label="Descripción" error={errors.message}>
                  <textarea className={`contact-input${errors.message ? " err" : ""}`} placeholder="Cuéntanos en detalle qué necesitas..." value={form.message} onChange={(e) => handleChange("message", e.target.value)} rows={4} style={{ resize: "vertical", minHeight: 100 }} />
                </Field>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  style={{ background: loading ? "rgba(124,58,237,0.5)" : "linear-gradient(135deg,#7c3aed,#a855f7)", border: "none", color: "#fff", padding: "13px", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Outfit', sans-serif", transition: "all .2s", marginTop: 4 }}
